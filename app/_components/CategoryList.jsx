    ...
    <div className='mx-4 md:mx-22 lg:mx-52 grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 sm:grid-cols-2 gap-4'>
        {
            categoryList.length>0?categoryList.map((category,index)=>(
                <Link  href={'/category/'+category.slug} key={index} className='flex flex-col items-center justify-center gap-2 p5 rounded-lg cursor-pointer hover:scale-110 transition-all ease-in-out'>
